...
def _maybe_tune_weak_payoff_from_review(review: dict, cfg: dict | None = None) -> dict:
    ...
    if not isinstance(state, dict):
        state = {}
    signature = f"weak_payoff:{review.get('label')}:{trades_n}:{round(payoff_ratio, 2)}"
    cooldown_sec = max(300, int(cfg.get("supervisorPayoffTuneCooldownMinutes", 45) or 45) * 60)
    if state.get("signature") == signature and now - int(state.get("at", 0) or 0) < cooldown_sec:
    ...
    set_float("holdMinConfidence", max(0.68, min(0.86, hold_min - 0.03)), 3)
    ...
    set_float("tpTargetMinUsdt", min(1.20, tp_min * 1.15), 3)
    set_float("tpTargetMaxUsdt", min(3.20, max(tp_max * 1.12, float(cfg.get("tpTargetMinUsdt", tp_min)) * 2.4)), 3)

    be_trigger = float(cfg.get("profitLockBreakevenTriggerUsdt", 0.16) or 0.16)
    set_float("profitLockBreakevenTriggerUsdt", min(0.45, max(0.20, be_trigger * 1.15)), 3)
    ...
    if weak_loss_pressure:
        ...
        set_float("supervisorSizeMultiplier", max(0.65, min(size_mult, 0.85)), 3)
    ...
    try:
        ...
        pass
    return {"applied": True, "signature": signature, "changes": changes}
...
def _external_signal_entry_guard_active() -> dict:
    ...
    now = int(time.time())
    active = (
        str(guard.get("reason", "") or "") == "marketcontext_mcp_data_failure"
...
def _is_marketcontext_no_data_error(message: str) -> bool:
...
def _maybe_tune_external_signal_guard(payload: dict | None, cfg: dict | None = None) -> dict:
    ...
    for item in findings:
        ...
        tv_signal = str(item.get("marketContextSignal") or item.get("tvSignal") or "").lower()
        data_failure = (
        ...
        unsupported_symbol = (
            "unsupported" in condition
            or "no_data" in condition
            or _is_marketcontext_no_data_error(tv_signal)
        )
        if unsupported_symbol:
            actionable.append({**item, "_kind": "unsupported_symbol"})
        elif data_failure:
    ...
    unsupported_items = [item for item in actionable if item.get("_kind") == "unsupported_symbol"]
        ...
        signature_parts.append(
            ":".join(
                [
                    ...
                    str(item.get("marketContextSignal") or item.get("tvSignal") or item.get("alignment") or "")[:32],
                ]
            )
        )
    ...
    cmux_action_report = None
    if unsupported_items:
        unsupported_symbols = [
            str(item.get("symbol", "") or "").upper().strip()
            for item in unsupported_items[:12]
            if str(item.get("symbol", "") or "").strip()
        ]
        if unsupported_symbols:
            deny = [str(x or "").upper().strip() for x in (cfg.get("scanDenySymbols") or []) if str(x or "").strip()]
            merged = []
            for sym in deny + unsupported_symbols:
                if sym and sym not in merged:
                    merged.append(sym)
            if merged != deny:
                cfg["scanDenySymbols"] = merged[:100]
                changes["scanDenySymbols"] = {"old": deny, "new": cfg["scanDenySymbols"]}
            lock_min = max(30, int(cfg.get("marketContextUnsupportedSymbolLockMinutes", 360) or 360))
            for sym in unsupported_symbols:
                _cooldown_scan_symbol(sym, lock_min * 60, "marketcontext_mcp_unsupported_symbol")
        cmux_action_report = {
            "type": "marketcontext_mcp_unsupported_symbol_skip",
            "action": "skip_symbols",
            "source": source,
            "symbols": unsupported_symbols,
            "note": "Symbols with no MarketContext MCP data were added to scanDenySymbols/cooldown; no global entry pause was applied.",
        }
        _agent_mark(
            "data_quality_guard",
            "done",
            "MarketContext unsupported symbols skipped",
            ", ".join(unsupported_symbols[:6]),
            {"source": source, "findings": unsupported_items[:6], "cmuxActionReport": cmux_action_report},
        )
    ...
    if data_failure_items:
        ...
        AUTO_TRADE["externalSignalGuard"] = {
            ...
            "source": source,
            "reason": "marketcontext_mcp_data_failure",
            "symbols": data_failure_symbols,
            "restoreConfig": restore_config,
        }
        cmux_action_report = {
            "type": "marketcontext_mcp_data_failure_guard",
            "action": "pause_new_entries",
            "until": AUTO_TRADE["pauseUntil"],
            "source": source,
            "symbols": AUTO_TRADE["externalSignalGuard"]["symbols"],
            "note": "Guardian can keep managing open positions; new entries are paused until MarketContext MCP recovers or the guard expires.",
        }
        set_bool("riskCooldownEnabled", True)
        set_bool("entryMarketContextBlockWeakSignal", True)
        set_bool("scanPerfSoftFallbackEnabled", False)
        ...
        if data_failure_symbols:
            ...
            for sym in data_failure_symbols:
                _cooldown_scan_symbol(sym, lock_min * 60, "marketcontext_mcp_no_data")
        ...
        _agent_mark(
            ...
            {"source": source, "findings": data_failure_items[:3], "cmuxActionReport": cmux_action_report},
        )
        _agent_mark(
            "hermes_supervisor",
            "blocked",
    ...
    if not changes:
        if cmux_action_report:
            AUTO_TRADE["lastCmuxReport"] = cmux_action_report
            AUTO_TRADE["config"] = cfg
            try:
                _persist_autotrade_snapshot()
            except Exception:
            ...
            return {
                ...
                "recorded": True,
                "signature": signature,
                "cmuxActionReport": cmux_action_report,
            }
    ...
    if data_failure_items:
        _agent_mark("risk_manager", "blocked", "external-signal entry pause active", source, {"changes": changes, "cmuxActionReport": cmux_action_report})
    elif pending_items:
    ...
    if cmux_action_report:
        out["cmuxActionReport"] = cmux_action_report
    return out
...
def _marketcontext_tool_text(result: dict | None) -> dict:
...
def _marketcontext_mcp_command(cfg: dict, port: int) -> list[str]:
    raw = cfg.get("marketContextWatcherCommand")
...
def _marketcontext_mcp_runtime_port(preferred_port: int) -> int:
    host = "127.0.0.1"
...
async def _marketcontext_mcp_post(client: httpx.AsyncClient, url: str, payload: dict, session_id: str | None = None) -> tuple[dict, str | None, int]:
    headers = {
...
async def _marketcontext_mcp_call_batch(calls: list[dict], cfg: dict) -> list[dict]:
    if not calls:
        return []
    preferred_port = int(cfg.get("marketContextWatcherMcpPort", 8877) or 8877)
    port = _marketcontext_mcp_runtime_port(preferred_port)
    timeout_sec = max(8.0, min(60.0, float(cfg.get("marketContextWatcherMcpCallTimeoutSec", 18) or 18)))
    startup_sec = max(8.0, min(45.0, float(cfg.get("marketContextWatcherMcpStartupSec", 25) or 25)))
    url = f"http://127.0.0.1:{port}/mcp"
    proc = None
    try:
        proc = await asyncio.create_subprocess_exec(
            *_marketcontext_mcp_command(cfg, port),
        ...
        async with httpx.AsyncClient(timeout=timeout_sec) as client:
            init_payload = {
                ...
                "params": {
                    ...
                    "capabilities": {},
                    "clientInfo": {"name": "hermes-marketcontext-watcher", "version": "1"},
                },
            }
            init_body = {}
            while time.time() < deadline:
                if proc.returncode is not None:
                    raise RuntimeError(f"MarketContext MCP exited during initialize on port {port} (code {proc.returncode})")
                try:
                    init_body, session_id, status = await _marketcontext_mcp_post(client, url, init_payload)
                    if status == 200 and init_body.get("result"):
                        break
                except Exception:
                    await asyncio.sleep(1.0)
            if not init_body.get("result"):
                raise RuntimeError(f"MarketContext MCP initialize timeout on port {port} (preferred {preferred_port})")
            await _marketcontext_mcp_post(
                client,
                url,
            ...
            for call in calls:
                ...
                try:
                    body, session_id, _status = await _marketcontext_mcp_post(client, url, payload, session_id)
                    result = body.get("result") if isinstance(body, dict) else {}
                    out.append({**call, "ok": bool(result), "data": _marketcontext_tool_text(result), "raw": body, "mcpPort": port, "preferredMcpPort": preferred_port})
...
def _marketcontext_target_symbols(state: dict, cfg: dict) -> list[dict]:
    max_symbols = max(1, min(8, int(cfg.get("marketContextWatcherMaxSymbols", 3) or 3)))
    targets: list[dict] = []
    seen: set[str] = set()
    denied_symbols = _parse_symbol_whitelist(cfg.get("scanDenySymbols"))

    def add(symbol: str, side: str = "", target: str = "pending_entry"):
        sym = str(symbol or "").upper().strip()
        if not sym or sym in seen or len(targets) >= max_symbols:
            return
        if sym in denied_symbols and bool(cfg.get("marketContextWatcherSkipDeniedSymbols", True)):
            return
        seen.add(sym)
    ...
    if bool(cfg.get("marketContextWatcherOpenPositionsOnly", False)) and targets:
        return targets
    ...
    if not targets:
        fallback = str(cfg.get("marketContextWatcherFallbackSymbol") or cfg.get("benchmarkSymbol") or "BTCUSDT").upper().strip()
...
def _marketcontext_calls_for_targets(targets: list[dict], cfg: dict) -> list[dict]:
    exchange = str(cfg.get("marketContextWatcherExchange") or "BINANCE").upper()
    timeframe = str(cfg.get("marketContextWatcherTimeframe") or "1h")
    use_mtf = bool(cfg.get("marketContextWatcherUseMtf", True))
    use_volume = bool(cfg.get("marketContextWatcherUseVolume", True))
    calls: list[dict] = []
    for target in targets:
        ...
        if use_volume and target.get("target") == "pending_entry":
...
def _marketcontext_alignment_status(data: dict | None) -> str:
    ...
    if status:
        return status
    sentiment = data.get("market_sentiment") if isinstance(data.get("market_sentiment"), dict) else {}
    signal = str(sentiment.get("buy_sell_signal") or "").upper()
...
def _marketcontext_findings_from_results(targets: list[dict], results: list[dict], state: dict) -> list[dict]:
    ...
    for target in targets:
        ...
        mtf = by_symbol.get(sym, {}).get("multi_timeframe_analysis", {}) or by_symbol.get(sym, {}).get("coin_analysis", {})
        status = _marketcontext_alignment_status(mtf)
        ...
        if errors.get(sym):
            ...
            error_text = "; ".join(errors[sym])[:240]
            unsupported = _is_marketcontext_no_data_error(error_text)
            findings.append({
                "symbol": sym,
                "side": side,
                "target": entry_target,
                ...
                "marketContextSignal": error_text,
                "hermesState": (
                    "MarketContext MCP has no data for this symbol"
                    if unsupported
                    else (
                        "MarketContext MCP data unavailable for active risk context"
                        if target_kind in {"open_position", "guardian_active_position"}
                        else "MarketContext MCP data unavailable for pending entry validation"
                    )
                ),
                "recommendedAction": (
                    ...
                    if unsupported
                        ...
                        if target_kind in {"open_position", "guardian_active_position"}
                        else "Block this entry until MarketContext context recovers or symbol is excluded from TV-gated scans."
                    )
                ),
            })
            continue
        ...
        if target_kind == "open_position" and side_contradicted:
            ...
            findings.append({
                ...
                "marketContextSignal": status,
                "alignment": json.dumps(mtf.get("alignment", {}), ensure_ascii=False)[:500],
                "hermesState": "Hermes is considering a new entry while MarketContext MTF leans the other way",
            ...
            findings.append({
                ...
                "marketContextSignal": status,
                "alignment": json.dumps(mtf.get("alignment", {}), ensure_ascii=False)[:500],
                "hermesState": "Hermes is considering a new entry while MarketContext MTF is mixed/ranging",
            ...
            findings.append({
                ...
                "marketContextSignal": status,
                "alignment": json.dumps(mtf.get("alignment", {}), ensure_ascii=False)[:500],
                "hermesState": "scanBoard pending entry with MarketContext volume confirmation",
                "recommendedAction": "Allow Supervisor to relax scan fallback within bounded policy.",
            })
    return findings


async def _marketcontext_signal_watch_once(provider=None) -> dict:
    cfg = AUTO_TRADE.get("config") if isinstance(AUTO_TRADE.get("config"), dict) else {}
    if not bool(cfg.get("marketContextWatcherEnabled", False)):
    ...
    targets = _marketcontext_target_symbols(state, cfg)
    if not targets:
        return {"ok": True, "enabled": True, "submitted": False, "reason": "no_targets"}
    calls = _marketcontext_calls_for_targets(targets, cfg)
    batch_timeout_sec = max(
        30.0,
        min(
            90.0,
            float(cfg.get("marketContextWatcherBatchTimeoutSec", 180.0) or 180.0),
        ),
    )
    try:
        coro = provider(calls, cfg) if provider else _marketcontext_mcp_call_batch(calls, cfg)
        results = await asyncio.wait_for(coro, timeout=batch_timeout_sec)
    except asyncio.TimeoutError as exc:
        timeout_msg = f"TimeoutError: marketcontext batch exceeded {batch_timeout_sec:.0f}s"
        failure_results = [
            {"ok": False, "symbol": target["symbol"], "name": "marketcontext_batch", "error": timeout_msg}
            for target in targets
        ]
        findings = _marketcontext_findings_from_results(targets, failure_results, state)
        supervisor_result = _maybe_tune_external_signal_guard({"source": "marketcontext_mcp_internal", "findings": findings}, cfg) if findings else None
        AUTO_TRADE["marketContextWatcher"] = {
        ...
        failure_results = [
            {"ok": False, "symbol": target["symbol"], "name": "marketcontext_batch", "error": error_msg}
            for target in targets
        ]
        findings = _marketcontext_findings_from_results(targets, failure_results, state)
        supervisor_result = _maybe_tune_external_signal_guard({"source": "marketcontext_mcp_internal", "findings": findings}, cfg) if findings else None
    ...
    findings = _marketcontext_findings_from_results(targets, results, state)
    payload = {"source": "marketcontext_mcp_internal", "findings": findings}
    ...
    guard = AUTO_TRADE.get("externalSignalGuard") if isinstance(AUTO_TRADE.get("externalSignalGuard"), dict) else {}
    if not has_data_failure and str(guard.get("reason", "") or "") == "marketcontext_mcp_data_failure":
        now = int(time.time())
        ...
        if isinstance(cfg, dict) and cfg_restore:
            ...
            for key, value in recovery_floor.items():
                old = cfg.get(key)
                if key == "maxOpenPositions":
                    value = max(int(old or 0), int(value))
                if old != value:
                    cfg[key] = value
                    restored_config[key] = {"old": old, "new": value}
            ...
            fallback_values = {
                "maxOpenPositions": max(3, int(cfg.get("supervisorTargetOpenPositionsMax", 6) or 6)),
                "minConfidence": float(cfg.get("marketContextRecoveryMinConfidence", 0.66) or 0.66),
                "scanFallbackNearEnabled": True,
                "scanPerfSoftFallbackEnabled": True,
            }
            for key, value in fallback_values.items():
                ...
                if old != value:
                    ...
                    restored_config[key] = {"old": old, "new": value}
        recovery_report = {
            "type": "marketcontext_mcp_recovered",
            "action": "clear_data_failure_entry_pause",
            "source": "marketcontext_mcp_internal",
            "symbols": guard.get("symbols") if isinstance(guard.get("symbols"), list) else [],
            "recoveredAt": now,
            "restoredConfig": restored_config,
        }
        AUTO_TRADE["externalSignalGuard"] = {**guard, "recoveredAt": now, "active": False}
        AUTO_TRADE["lastCmuxReport"] = recovery_report
        _agent_mark("data_quality_guard", "done", "MarketContext MCP recovered", "cleared external-signal data failure guard", recovery_report)
    ...
    if recovery_report:
        status["cmuxActionReport"] = recovery_report
    AUTO_TRADE["marketContextWatcher"] = status
    return status


async def _marketcontext_signal_watch_loop():
    while AUTO_TRADE.get("running") or AUTO_TRADE.get("manageOpenOnly"):
        cfg = AUTO_TRADE.get("config") if isinstance(AUTO_TRADE.get("config"), dict) else {}
        interval = max(60, int(cfg.get("marketContextWatcherIntervalSec", 300) or 300))
        try:
            await _marketcontext_signal_watch_once()
        except Exception as exc:
            AUTO_TRADE["marketContextWatcher"] = {
                "updatedAt": int(time.time()),
                "ok": False,
...
def _ensure_marketcontext_watcher_task():
    cfg = AUTO_TRADE.get("config") if isinstance(AUTO_TRADE.get("config"), dict) else {}
    if not bool(cfg.get("marketContextWatcherEnabled", False)):
        return
    global _TRADINGVIEW_WATCH_TASK
    if _TRADINGVIEW_WATCH_TASK is None or _TRADINGVIEW_WATCH_TASK.done():
        _TRADINGVIEW_WATCH_TASK = asyncio.create_task(_marketcontext_signal_watch_loop())


def _cancel_marketcontext_watcher_task():
    global _TRADINGVIEW_WATCH_TASK
...
def _maybe_tune_low_entry_activity(reason: str, cfg: dict | None = None, board: list[dict] | None = None) -> dict:
    ...
    if stuck_no_entry:
        ...
        set_float("minConfidence", max(min_floor, min_conf - 0.015), 3)
        set_float("earlyEntryMinConfidence", max(early_floor, early_conf - 0.015), 3)
        ...
        set_float("minConfidence", max(0.48 if quiet_board else 0.50, min_conf - (0.03 if quiet_board else 0.02)), 3)
        set_float("earlyEntryMinConfidence", max(0.50 if quiet_board else 0.52, early_conf - (0.025 if quiet_board else 0.015)), 3)
    if quiet_board and not stuck_no_entry:
        score_gap = float(cfg.get("earlyEntryScoreGapMin", 1.40) or 1.40)
        set_float("earlyEntryScoreGapMin", max(0.90, score_gap - 0.12), 3)
        hybrid_score = float(cfg.get("hybridMinScore", 0.72) or 0.72)
        set_float("hybridMinScore", max(0.62, hybrid_score - 0.025), 3)
        hybrid_edge = float(cfg.get("hybridMinEdge", 0.06) or 0.06)
        set_float("hybridMinEdge", max(0.025, hybrid_edge - 0.01), 3)
    analyze_top = max(3, int(cfg.get("scanAnalyzeTop", 8) or 8))
    set_int("scanAnalyzeTop", min(16, analyze_top + 2))
...
def _maybe_tune_scan_timeout_from_skip(skip_msg: str, cfg: dict | None = None) -> dict:
    ...
    set_float("scanPerSymbolTimeoutSec", min(10.0, per_symbol_timeout * 1.15), 2)
    analyze_top = max(3, int(cfg.get("scanAnalyzeTop", 8) or 8))
    set_int("scanAnalyzeTop", max(3, analyze_top - 1))
    guarded_top = max(analyze_top, int(cfg.get("scanGuardedFallbackAnalyzeTop", max(analyze_top * 2, analyze_top + 4)) or analyze_top))
    set_int("scanGuardedFallbackAnalyzeTop", max(int(cfg.get("scanAnalyzeTop", analyze_top) or analyze_top), guarded_top - 2))
    fallback_retries = max(1, int(cfg.get("scanFallbackRetrySymbols", 3) or 3))
    set_int("scanFallbackRetrySymbols", max(1, fallback_retries - 1))

    if not changes:
        return {"applied": False, "reason": "no_safe_delta"}
    return _commit_supervisor_config_tune(state, delegations, "scan_timeout", cfg, changes, str(skip_msg or "scan timeout"))
...
def _maybe_tune_daily_entry_regression(daily_review: dict, cfg: dict | None = None) -> dict:
    ...
    signature = (
        f"{today.get('day')}:{today.get('trades')}:{today.get('winRatePct')}:"
        f"{today.get('pnl')}:{baseline.get('day')}:{baseline.get('pnl')}"
    )
    ...
    set_float("minConfidence", min(0.84, max(min_conf, min_conf + 0.015)), 3)
    early_conf = float(cfg.get("earlyEntryMinConfidence", 0.60) or 0.60)
    set_float("earlyEntryMinConfidence", min(0.76, max(early_conf, early_conf + 0.015)), 3)
    gap = float(cfg.get("earlyEntryScoreGapMin", 1.40) or 1.40)
    set_float("earlyEntryScoreGapMin", min(2.10, max(gap, gap + 0.12)), 3)
    ...
    set_float("riskCooldownResumeScoreGapMin", min(2.60, max(float(cfg.get("riskCooldownResumeScoreGapMin", 2.0) or 2.0), 2.20)), 3)
    set_float("maxSpreadBps", max(10.0, min(float(cfg.get("maxSpreadBps", 22.0) or 22.0), float(cfg.get("maxSpreadBps", 22.0) or 22.0) * 0.90)), 2)
    if not bool(cfg.get("scanFallbackNearEnabled", True)):
        changes["scanFallbackNearEnabled"] = {"old": False, "new": True}
        cfg["scanFallbackNearEnabled"] = True
    near_relax = float(cfg.get("scanFallbackNearConfRelax", 0.04) or 0.04)
    set_float("scanFallbackNearConfRelax", max(0.02, min(near_relax, 0.035)), 3)
    guarded_relax = float(cfg.get("scanGuardedFallbackConfRelax", 0.04) or 0.04)
    set_float("scanGuardedFallbackConfRelax", max(0.02, min(guarded_relax, 0.04)), 3)
    ...
    set_float("todayPerformanceGuardMaxWinRatePct", min(48.0, max(float(cfg.get("todayPerformanceGuardMaxWinRatePct", 40.0) or 40.0), 45.0)), 2)
...
def _maybe_tune_small_profit_capture_from_review(review: dict, cfg: dict | None = None) -> dict:
    ...
    set_float("holdMinConfidence", max(0.66, min(0.84, hold_min - 0.02)), 3)
    trigger = float(cfg.get("profitLockTriggerUsdt", 0.35) or 0.35)
    keep = float(cfg.get("profitLockKeepUsdt", 0.15) or 0.15)
    giveback = float(cfg.get("profitLockMaxGivebackUsdt", 0.22) or 0.22)
    set_float("profitLockTriggerUsdt", max(0.22, min(trigger, trigger * 0.85)), 3)
    set_float("profitLockKeepUsdt", min(0.45, max(keep, keep * 1.20, 0.18)), 3)
    set_float("profitLockMaxGivebackUsdt", max(0.10, min(giveback, giveback * 0.82, 0.18)), 3)
    floor = float(cfg.get("profitLockBreakevenFloorUsdt", 0.08) or 0.08)
    set_float("profitLockBreakevenFloorUsdt", min(0.18, max(floor, 0.10)), 3)
    tp_min = max(0.20, float(cfg.get("tpTargetMinUsdt", 0.55) or 0.55))
    ...
    set_float("profitLockBreakevenTriggerUsdt", min(float(cfg.get("profitLockTriggerUsdt", trigger) or trigger), max(0.14, breakeven_trigger * 0.95)), 3)

    if not changes:
        if active:
            return {"applied": False, "alreadyTuned": True, "cooldownSec": cooldown_sec}
        return {"applied": False, "reason": "no_safe_delta"}
    return _commit_supervisor_config_tune(state, delegations, "small_profit_capture", cfg, changes, str(review.get("label") or "small wins"))
...
def _maybe_tune_negative_expectancy_from_review(review: dict, cfg: dict | None = None) -> dict:
    ...
    signature = f"{label}:{trades_n}:{round(win_rate, 1)}:{round(avg_pnl, 3)}:{round(total_pnl, 3)}"
    ...
    set_float("minConfidence", min(0.84, max(min_conf, min_conf + min(conf_step, 0.015))), 3)

    early_conf = float(cfg.get("earlyEntryMinConfidence", 0.60) or 0.60)
    set_float("earlyEntryMinConfidence", min(0.76, max(early_conf, early_conf + 0.015)), 3)
    gap = float(cfg.get("earlyEntryScoreGapMin", 1.40) or 1.40)
    set_float("earlyEntryScoreGapMin", min(2.10, max(gap, gap + (0.12 if quick_losses >= 2 else 0.08))), 3)
    ...
    hybrid_score = float(cfg.get("hybridMinScore", 0.72) or 0.72)
    set_float("hybridMinScore", min(0.86, max(hybrid_score, hybrid_score + 0.02)), 3)
    hybrid_edge = float(cfg.get("hybridMinEdge", 0.06) or 0.06)
    set_float("hybridMinEdge", min(0.12, max(hybrid_edge, hybrid_edge + 0.01)), 3)

    max_spread = float(cfg.get("maxSpreadBps", 22.0) or 22.0)
    set_float("maxSpreadBps", max(12.0, min(max_spread, max_spread * 0.92)), 2)
    ...
    set_float("todayPerformanceGuardMaxWinRatePct", min(45.0, max(guard_wr, 40.0)), 2)
    ...
    set_float("sessionBiasBadWinRatePct", min(48.0, max(bad_wr, 45.0)), 2)
    max_shift = float(cfg.get("sessionBiasMaxConfShift", 0.05) or 0.05)
    set_float("sessionBiasMaxConfShift", min(0.06, max(max_shift, 0.05)), 3)
...
def _maybe_tune_size_multiplier_from_streak(trades: list[dict], cfg: dict | None = None) -> dict:
    ...
    signature = f"{reason}:{state.get('signature')}:{target:.3f}"
...
def _live_closed_trades_from_log(symbol: str | None = None, mode: str = "ALL") -> list[dict]:
    ...
    for line in lines:
        ...
        if not math.isfinite(pnl):
            continue
        # Ignore impossible historical anomalies.
        if abs(pnl) > 5000.0:
            continue
        ts_raw = obj.get("closedAt", obj.get("ts", 0))
        try:
...
async def _manage_live_open_positions_once(cfg: dict, now: int) -> bool:
    ...
    if closed:
        _agent_mark("position_guardian", "done", "position closed by guardian")
        AUTO_TRADE["lastTradeAt"] = now
        AUTO_TRADE["trades"].append(now)
        return True
    locks = AUTO_TRADE.get("liveProfitLocks") if isinstance(AUTO_TRADE.get("liveProfitLocks"), dict) else {}
    guardian = AUTO_TRADE.get("liveGuardian") if isinstance(AUTO_TRADE.get("liveGuardian"), dict) else {}
    if locks or bool(guardian.get("active")):
...
async def _lifespan(app: FastAPI):
    ...
    try:
        ...
        _cancel_marketcontext_watcher_task()
        _persist_autotrade_snapshot()
...
def _ensure_autotrade_task_alive(reason: str = "watchdog") -> bool:
    ...
    _ensure_marketcontext_watcher_task()
...
def _symbol_perf_gate(cfg: dict, symbol: str) -> tuple[bool, str, dict]:
    ...
    pr = _load_learning_profiles().get(sym, {})
...
def _loss_streak_self_review_tune(cfg: dict, now: int, loss_streak: int, cause: dict | None = None) -> dict:
    ...
    new_conf = min(0.86, max(old_conf, old_conf + 0.02))
    if new_conf > old_conf:
        out["minConfidence"] = round(new_conf, 4)
        actions.append(f"minConfidence {old_conf:.2f}->{new_conf:.2f}")
    if out.get("scanSidePreference") != "score":
        out["scanSidePreference"] = "score"
        actions.append("scanSidePreference=score")
    if bool(out.get("scanFallbackNearEnabled", True)):
        out["scanFallbackNearEnabled"] = False
        actions.append("scanFallbackNearEnabled=false")
    if not bool(out.get("pairLockEnabled", True)):
        out["pairLockEnabled"] = True
        actions.append("pairLockEnabled=true")
    out["pairLockLossStreak"] = min(int(out.get("pairLockLossStreak", 2) or 2), 2)
    out["pairLockMinutes"] = max(int(out.get("pairLockMinutes", 45) or 45), 90)
    out["riskCooldownEnabled"] = True
    out["riskCooldownLossStreak"] = min(int(out.get("riskCooldownLossStreak", 3) or 3), 3)
    out["riskCooldownMinutes"] = max(int(out.get("riskCooldownMinutes", 25) or 25), 45)
    out["perfGateEarlyMinSamples"] = min(int(out.get("perfGateEarlyMinSamples", 4) or 4), 4)
    out["perfGateEarlyMinPnlUsdt"] = max(float(out.get("perfGateEarlyMinPnlUsdt", -0.35) or -0.35), -0.35)
    out["perfLockMinutes"] = max(int(out.get("perfLockMinutes", 90) or 90), 120)
    old_max_open = int(out.get("maxOpenPositions", 6) or 6)
    diversification_floor = 6 if (bool(out.get("marketScan")) or str(out.get("symbol", "")).upper() in {"AUTO", "SCAN"}) else 2
    new_max_open = max(diversification_floor, min(old_max_open, old_max_open - 1 if loss_streak >= 4 else old_max_open))
    if new_max_open < old_max_open:
        out["maxOpenPositions"] = new_max_open
        actions.append(f"maxOpenPositions {old_max_open}->{new_max_open}")
    elif old_max_open < diversification_floor:
        out["maxOpenPositions"] = diversification_floor
    ...
    if isinstance(st, dict):
        ...
        if trades >= int(out.get("selfReviewMinHourSamples", 8) or 8) and wr <= 42.0 and pnl < 0:
            ...
            windows = list(out.get("noTradeWindows") or [])
...
def _entry_snapshot_for_position(symbol: str, side: str) -> dict:
    ...
    guardian = AUTO_TRADE.get("liveGuardian")
    if isinstance(guardian, dict):
        gsym = str(guardian.get("symbol", "") or "").upper().strip()
        gside = str(guardian.get("side", "") or "").upper().strip()
        if gsym == sym and gside == sd and isinstance(guardian.get("entrySnapshot"), dict):
            return dict(guardian.get("entrySnapshot") or {})
...
def _intel_data_quality_guard(intel: dict | None) -> dict:
    ...
    blocking = [x for x in issues if x not in {"spread_missing", "precision_missing", "momentum_missing"}]
    return {
        "ok": not blocking,
...
def _strong_reversal_structure_confirmed(current_side: str, opposite: str, precision: dict | None, cfg: dict) -> tuple[bool, str]:
    ...
    px = precision if isinstance(precision, dict) else {}
    if not px:
        return True, "structure=unavailable"
...
def _latest_marketcontext_finding(symbol: str, max_age_sec: int) -> dict:
    tvw = AUTO_TRADE.get("marketContextWatcher") if isinstance(AUTO_TRADE.get("marketContextWatcher"), dict) else {}
...
def _entry_marketcontext_context(symbol: str, side: str, cfg: dict) -> dict:
    if not bool(cfg.get("entryMarketContextGateEnabled", True)):
        return {"active": False}
    current_side = str(side or "").upper()
    if current_side not in ("LONG", "SHORT"):
        return {"active": False}
    finding = _latest_marketcontext_finding(symbol, int(cfg.get("entryMarketContextGateMaxAgeSec", 420) or 420))
    if not finding:
        return {"active": False}
    ...
    signal = str(finding.get("marketContextSignal") or finding.get("alignment") or "").upper()
    ...
    block = data_failure or (bool(cfg.get("entryMarketContextBlockContradiction", True)) and contradiction)
    if weak and bool(cfg.get("entryMarketContextBlockWeakSignal", False)):
        block = True
    return {
        "active": True,
        "finding": finding,
        ...
        "block": bool(block),
        "confidenceBoost": float(cfg.get("entryMarketContextConfirmConfidenceBoost", 0.015) or 0.0) if confirmed and not contradiction and not weak and not data_failure else 0.0,
        "extraConfidence": float(cfg.get("entryMarketContextWeakExtraConfidence", 0.03) or 0.0) if weak and not block else 0.0,
    }


def _guardian_marketcontext_context(symbol: str, side: str, mark: float, entry: float, qty: float, sl: float, tp: float, cfg: dict) -> dict:
    if not bool(cfg.get("guardianMarketContextContextEnabled", True)):
        return {"active": False}
    current_side = str(side or "").upper()
    if current_side not in ("LONG", "SHORT"):
        return {"active": False}
    finding = _latest_marketcontext_finding(symbol, int(cfg.get("guardianMarketContextContextMaxAgeSec", 420) or 420))
    if not finding:
        return {"active": False}
    condition = str(finding.get("condition") or "").lower()
    target = str(finding.get("target") or "").lower()
    signal = str(finding.get("marketContextSignal") or "").upper()
    ...
    if mark <= 0 or entry <= 0 or qty <= 0 or sl <= 0 or tp <= 0:
        return {"active": False, "finding": finding}
    tighten_pct = max(0.05, min(1.0, float(cfg.get("guardianMarketContextTightenSlPct", 0.22) or 0.22)))
    min_lock_usdt = max(0.0, float(cfg.get("guardianMarketContextMinProfitLockUsdt", 0.06) or 0.06))
    ...
    return {
        ...
        "blockTpExtension": bool(cfg.get("guardianMarketContextBlockTpExtension", True)),
...
async def _live_guardian_maybe_close(cfg: dict):
    ...
    tv_guard = _guardian_marketcontext_context(symbol, side, mark, entry, qty, sl, tp, cfg)
    if tv_guard.get("active"):
        g["marketContextContext"] = {
            "updatedAt": int(time.time()),
        ...
        if tv_guard.get("changedSl"):
            ...
            g["sl"] = float(sl)
            g["marketContextContext"]["tightenedSl"] = round(float(sl), 10)
            _autotrade_log(f"LIVE guardian MarketContext tighten SL: {symbol} {side} SL={sl:.6f} {tv_guard.get('signal')}")
    ...
    if entry > 0 and qty > 0:
        ...
        if bool(payoff_guard.get("active")) and upnl <= -float(payoff_guard.get("maxLossUsdt", 0.0) or 0.0):
            hit_payoff_loss_guard = True
            g["payoffLossGuard"] = payoff_guard
        peak = float(g.get("peakUnrealizedPnl", upnl) or upnl)
        if upnl > peak:
            peak = upnl
        ...
        if peak >= lock_trigger:
            ...
            if side == "LONG":
                ...
                sl = min(sl, lock_sl)
            g["sl"] = float(sl)
            g["profitLockArmed"] = True
            g["profitLockUsdt"] = round(float(lock_usdt), 6)
            g["profitLockAt"] = int(time.time())
        if peak >= breakeven_trigger and 0 < upnl <= breakeven_floor:
            hit_breakeven_guard = True
            g["breakevenGuardArmed"] = True
            g["breakevenFloorUsdt"] = round(float(breakeven_floor), 6)
    ...
    reason = (
        "LOCAL_TP_HIT"
        if hit_tp
        else "BREAKEVEN_GUARD"
        if hit_breakeven_guard
        else "STRONG_REVERSAL_EXIT"
        if reversal_exit
        else "PAYOFF_LOSS_GUARD"
    ...
    extra_reason = f" · {reversal_reason}" if reversal_exit and reversal_reason else ""
    if hit_payoff_loss_guard:
        extra_reason = (
            f" · cap={float(payoff_guard.get('maxLossUsdt', 0.0) or 0.0):.3f}"
            f" payoff={float(payoff_guard.get('payoffRatio', 0.0) or 0.0):.2f}"
        )
    _autotrade_log(f"LIVE guardian close: {reason} at {mark:.6f}{extra_reason}")
    return True
...
async def _live_multi_profit_lock_manage(cfg: dict) -> bool:
    ...
    for p in rows:
        ...
        tv_guard = _guardian_marketcontext_context(sym, side, mark, guard_entry, float(p.get("qty", 0.0) or 0.0), sl, tp, cfg)
        if tv_guard.get("active"):
            st["marketContextContext"] = {
                "updatedAt": now,
                "signal": tv_guard.get("signal"),
                "condition": tv_guard.get("condition"),
                "blockTpExtension": bool(tv_guard.get("blockTpExtension")),
            ...
            if tv_guard.get("changedSl"):
                sl = float(tv_guard.get("tightenedSl") or sl)
                st["sl"] = round(float(sl), 10)
                st["marketContextContext"]["tightenedSl"] = round(float(sl), 10)
                _autotrade_log(f"LIVE multi guard MarketContext tighten SL: {sym} {side} SL={sl:.6f} {tv_guard.get('signal')}")
        ...
        hit_payoff_loss_guard = bool(payoff_guard.get("active")) and upnl <= -float(payoff_guard.get("maxLossUsdt", 0.0) or 0.0)
        if hit_payoff_loss_guard:
            st["payoffLossGuard"] = payoff_guard
        if st["peak"] >= breakeven_trigger:
            st["breakevenGuardArmed"] = True
            st["breakevenFloorUsdt"] = round(float(breakeven_floor), 6)
        if mark > 0 and tp > 0 and sl > 0:
            hit_sl = (side == "LONG" and mark <= sl) or (side == "SHORT" and mark >= sl)
            hit_breakeven_guard = bool(st.get("breakevenGuardArmed")) and 0 < upnl <= breakeven_floor
            if hit_breakeven_guard or hit_sl:
                await _close_position_one_side(sym, side, key, secret, base)
                reason = "BREAKEVEN_GUARD" if hit_breakeven_guard else "LOCAL_SL_HIT"
        ...
        intel = None
        try:
            intel = await asyncio.wait_for(intel_analyze(IntelAnalyzeRequest(symbol=sym)), timeout=6.0)
        except Exception:
            intel = None
        strong_follow = False
        weak_now = False
        if isinstance(intel, dict):
            ...
            if tv_guard.get("active") and tv_guard.get("blockTpExtension"):
                strong_follow = False
                follow_reason = f"MarketContext {tv_guard.get('signal')} blocks TP extension"
        ...
        if mark > 0 and tp > 0 and sl > 0:
            ...
            if hit_tp and strong_follow:
                ...
                if extended:
                    ...
                    _autotrade_log(
                        f"LIVE multi guard extend TP: {sym} {side} TP={new_tp:.6f} SL={new_sl:.6f} · {follow_reason}"
                    )
                    locks[k] = st
                    changed = True
                    continue
            if hit_tp or hit_breakeven_guard or hit_sl:
                await _close_position_one_side(sym, side, key, secret, base)
                reason = (
                    "LOCAL_TP_HIT"
                    if hit_tp
...
def _persist_autotrade_snapshot():
    try:
        payload = {
            ...
            "marketContextWatcher": AUTO_TRADE.get("marketContextWatcher") if isinstance(AUTO_TRADE.get("marketContextWatcher"), dict) else {},
            "externalSignalGuard": AUTO_TRADE.get("externalSignalGuard") if isinstance(AUTO_TRADE.get("externalSignalGuard"), dict) else {},
            "lastCmuxReport": AUTO_TRADE.get("lastCmuxReport") if isinstance(AUTO_TRADE.get("lastCmuxReport"), dict) else {},
...
def _load_autotrade_snapshot():
    ...
    try:
        ...
        tvw = data.get("marketContextWatcher")
        AUTO_TRADE["marketContextWatcher"] = tvw if isinstance(tvw, dict) else {}
        guard = data.get("externalSignalGuard")
        AUTO_TRADE["externalSignalGuard"] = guard if isinstance(guard, dict) else {}
        report = data.get("lastCmuxReport")
        AUTO_TRADE["lastCmuxReport"] = report if isinstance(report, dict) else {}
...
async def autotrade_update_config(payload: dict = Body(default_factory=dict)):
    ...
    if bool(cur.get("marketContextWatcherEnabled", False)):
        _ensure_marketcontext_watcher_task()
    else:
        _cancel_marketcontext_watcher_task()
    _autotrade_log(
...
async def autotrade_update_sl_tp(payload: dict = Body(default_factory=dict)):
    ...
    g = AUTO_TRADE.get("liveGuardian")
    if isinstance(g, dict) and g.get("active") and str(g.get("symbol", "")).upper() == symbol:
        side = str(g.get("side", "LONG")).upper()
        entry = float(g.get("entryMark", 0.0) or 0.0)
        if entry > 0:
            ...
            g["tp"] = tp_new
            g["sl"] = sl_new
            AUTO_TRADE["liveGuardian"] = g
            _autotrade_log(f"Update TP/SL {symbol}: TP={tp_new:.6f} SL={sl_new:.6f} ({side})")
    _persist_autotrade_snapshot()
    return {
        "ok": True,
        "updated": True,
    ...
    while AUTO_TRADE["running"] or AUTO_TRADE.get("manageOpenOnly"):
        ...
        try:
            ...
            if scan_mode:
                ...
                day_cap_scan = int(cfg.get("maxDailyTradesPerSymbol", 14) or 14)
                if day_cap_scan > 0:
            ...
            adaptive_min_conf = max(
                0.45,
                min(0.90, float(adaptive_min_conf) + float(session_bias.get("confidenceShift", 0.0) or 0.0)),
            )
            ...
            if signal not in ("LONG", "SHORT"):
                ...
                await asyncio.sleep(cfg["intervalSec"])
                continue
            tv_entry = _entry_marketcontext_context(str(cfg.get("symbol", "")), signal, cfg)
            if bool(tv_entry.get("active")):
                intel["marketContextEntryContext"] = tv_entry
                tv_detail = f"{tv_entry.get('condition') or 'context'} {tv_entry.get('signal') or ''}".strip()
                if bool(tv_entry.get("block")):
                    _agent_mark("strategy_builder", "blocked", "MarketContext entry gate", tv_detail)
                    _autotrade_skip(
                        "marketcontext_entry_gate",
                        f"Skip: MarketContext entry gate blocks {signal} {cfg['symbol']} ({tv_detail})",
                    )
                    await asyncio.sleep(cfg["intervalSec"])
                    continue
                ...
                if extra_conf > 0:
                    ...
                    _autotrade_log(
                        f"MarketContext entry gate: weak context raises min confidence by {extra_conf:.3f} ({tv_detail})"
                    )
                ...
                if conf_boost > 0:
                    ...
                    _autotrade_log(
                        f"MarketContext entry confirm: {cfg['symbol']} {signal} conf {old_conf:.3f}->{conf:.3f} ({tv_detail})"
                    )
            ...
            if mode == "PAPER":
                ...
                if external_guard.get("active"):
                    _agent_mark("data_quality_guard", "blocked", "MarketContext MCP guard blocks live entry", f"{cfg['symbol']} {signal}", external_guard)
                    _agent_mark("execution_agent", "blocked", "external-signal guard active", f"{cfg['symbol']} {signal}")
                    _autotrade_skip(
                        "external_signal_guard",
                        f"Skip: MarketContext MCP data guard active until {int(external_guard.get('until', 0) or 0)}",
                    )
                    await asyncio.sleep(cfg["intervalSec"])
                    continue
                ...
                external_guard = _external_signal_entry_guard_active()
                if external_guard.get("active"):
                    _agent_mark("data_quality_guard", "blocked", "MarketContext MCP guard blocks live order", f"{cfg['symbol']} {signal}", external_guard)
                    _agent_mark("execution_agent", "blocked", "external-signal guard active before order", f"{cfg['symbol']} {signal}")
                    _autotrade_skip(
                        "external_signal_guard",
                        f"Skip: MarketContext MCP data guard active before order until {int(external_guard.get('until', 0) or 0)}",
                    )
                    await asyncio.sleep(cfg["intervalSec"])
                    continue
            ...
            if _is_fapi_agreement_error(err_msg):
                ...
                _autotrade_log(
                    ...
                    + (f" · locked {blocked_symbol}" if blocked_symbol else "")
...
def _hermes_supervisor_review(bot_state: dict | None = None) -> dict:
    ...
    def active_open_positions() -> list[dict]:
        ...
        out: list[dict] = []
        guardian = bot.get("liveGuardian") if isinstance(bot.get("liveGuardian"), dict) else {}
        if guardian and bool(guardian.get("active")):
            out.append(guardian)
    ...
    for agent_id, agent in agents.items():
        ...
        if state_name == "blocked":
            ...
            add_issue(
                ...
                suggestion,
                None if (is_capacity_hold or is_safety_hold or is_strategy_safety_hold or is_market_safety_hold) else f"Inspect repeated blocked state for {agent_id}: {action}",
            )
            ...
            add_issue(
                ...
                f"Add timeout/heartbeat around long-running {agent_id} action",
            )
    ...
    if runs_by_agent:
        ...
        if max_agent == "hermes_supervisor":
            ...
            add_issue(
                ...
                f"Review workload split around {max_agent}",
            )
    ...
    if not safety_hold_active and consecutive_errors >= 2 and not known_operator_hold:
        add_issue(
            ...
            "Trace AUTO_TRADE consecutiveErrors and add targeted recovery",
        )
    ...
    if not safety_hold_active and is_scan_timeout_skip:
        ...
        add_issue(
            ...
            None if scan_timeout_handled else "Tune market scan timeout handling and symbol backoff",
    ...
    if scan_review_enabled and all(str(row.get("rejectReason", "") or "").lower() == "analyze_error" for row in board if isinstance(row, dict)):
        add_issue(
            ...
            "Debug analyze_error path for market scan board",
        )
    ...
    if not safety_hold_active and running and execution_mode == "LIVE" and len(scan_picks) >= 3:
        ...
        if top_count >= 3:
            ...
            add_issue(
                ...
                "Fix scan candidate lock enforcement for repeated rejected symbol picks" if locked_pick else None,
            )
            add_auto_action(
                "market_analyst",
    ...
    if not safety_hold_active and running and execution_mode == "LIVE" and scan_mode_cfg and not board_rows:
        ...
        if int(ma.get("runs", 0) or 0) >= 2 and ma_age > interval * 3 and ma_action not in {"scan market candidates", "waiting"}:
            add_issue(
                ...
                "Debug AUTO scan board lifecycle when scanBoard stays empty",
            )
    ...
    if scan_review_enabled and board_rows:
        ...
        if no_qualified_scan_candidates and skip_code == "scan_none" and bot is AUTO_TRADE and running and execution_mode == "LIVE":
            ...
            live_max_open = max(1, int(cfg.get("maxOpenPositions", 6) or 6))
        ...
        if perf_locks_are_root_cause:
            ...
            add_issue(
                ...
                "Review perf_lock thresholds and symbol universe after repeated low entries",
        ...
        if no_qualified_scan_candidates:
            ...
            add_issue(
                ...
                None if scan_none_tune_handled else "Review scan fallback when all candidates are guarded out",
    ...
    max_open_positions = max(1, int(cfg.get("maxOpenPositions", 6) or 6))
    ...
    guardian_stale = open_positions and (guardian_state == "todo" or agent_age("position_guardian") > 90)
    if guardian_stale:
        add_issue(
            "position_guardian",
            "high",
            ...
            "Wire Position Guardian heartbeat to openLivePositions source",
        )
        add_auto_action(
            "position_guardian",
    ...
    if latest_trade_review and not safety_hold_active and strategy_trade_review_allowed:
        ...
        if daily_regime_review.get("degraded"):
            ...
            add_issue(
                ...
                None if (daily_tune.get("applied") or daily_tune.get("alreadyTuned")) else "Compare today's entry behavior against profitable baseline days and tune entry filters",
        ...
        if trades_n >= 6 and total_pnl < 0 and (win_rate < 45.0 or avg_pnl < -0.04) and not symbol_drag_explains_window:
            ...
            add_issue(
                ...
                None if (negative_tune.get("applied") or negative_tune.get("alreadyTuned")) else "Analyze recent negative expectancy window and tune entry filters, symbol selection, and session bias",
        ...
        if weak_payoff_ratio and not symbol_drag_explains_window:
            ...
            add_issue(
                ...
                None if payoff_tune_handled else "Review TP/SL, hold-winner, and profit-lock thresholds because payoff ratio is weak",
        ...
        if trades_n >= 6 and quick_losses >= max(2, math.ceil(trades_n * 0.25)):
            add_issue(
                ...
                "Trace quick-loss trades and tune early-entry, anti-chase, and HTF conflict filters",
                supervisor_first=True,
            )
        ...
        if trades_n >= 6 and small_win_cluster and small_win_payoff_risk:
            ...
            add_issue(
                ...
                None if small_win_tune_handled else "Review profit-lock trigger, keep floor, max giveback, and weak-signal exits for small-profit clustering",
        ...
        if symbol_drag_explains_window:
            ...
            add_issue(
                ...
                None if symbol_drag_handled else f"Review symbol-specific filters and perf lock behavior for {sym}",
            )
    ...
    if not safety_hold_active and len(small_profit_exits) >= 3:
        ...
        add_issue(
            ...
            None if small_profit_exit_handled else "Review guardian early-exit and hold-winner thresholds",
    ...
    if not safety_hold_active and trade_count >= 3 and memory_runs == 0:
        add_issue(
            ...
            "Wire Memory Agent confirmation after live trade close",
            supervisor_first=True,
        )
    ...
    if not safety_hold_active and trade_count >= 6 and reflection_runs == 0:
        add_issue(
            ...
            "Wire Reflection Agent after enough closed trades",
            supervisor_first=True,
        )
    ...
    if not safety_hold_active and trade_count >= 8 and backtest_runs == 0:
        add_issue(
            ...
            "Wire Backtest Agent validation for learning config proposals",
            supervisor_first=True,
        )
...
@app.get("/hermes/marketcontext/status")
def hermes_marketcontext_status():
    cfg = AUTO_TRADE.get("config") if isinstance(AUTO_TRADE.get("config"), dict) else {}
    return {
        "ok": True,
        "enabled": bool(cfg.get("marketContextWatcherEnabled", False)),
        "running": bool(AUTO_TRADE.get("running")),
        "manageOpenOnly": bool(AUTO_TRADE.get("manageOpenOnly")),
        "pauseUntil": int(AUTO_TRADE.get("pauseUntil", 0) or 0),
        "marketContextWatcher": AUTO_TRADE.get("marketContextWatcher") if isinstance(AUTO_TRADE.get("marketContextWatcher"), dict) else {},
...
@app.post("/hermes/marketcontext/refresh")
async def hermes_marketcontext_refresh(wait: bool = True):
    if not wait:
        asyncio.create_task(_marketcontext_signal_watch_once())
        return {"ok": True, "scheduled": True}
    try:
        out = await asyncio.wait_for(_marketcontext_signal_watch_once(), timeout=180.0)
    except Exception as exc:
...
def autotrade_status_lite():
    ...
    return {
        ...
        "config": {
            ...
            "marketContextWatcherEnabled": public_cfg.get("marketContextWatcherEnabled"),
            "marketContextWatcherIntervalSec": public_cfg.get("marketContextWatcherIntervalSec"),
        },
        "riskLimits": {
            "maxLeverage": RISK["max_leverage"],
            "maxNotionalUSDT": RISK["max_notional"],
...
def learning_status(symbol: str | None = None):
    ...
    profiles = _load_learning_profiles()
    out = []
    symbols = sorted({str(k).upper() for k in (profiles or {}).keys() if str(k).strip()})
    stats_by_symbol = _aggregate_live_trade_stats_by_symbol_from_log()
    symbols.extend(stats_by_symbol.keys())
