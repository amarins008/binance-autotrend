// CmuxPanel.tsx — 4-tab professional UI
...
const Card = ({ children, style }: { children: React.ReactNode; style?: React.CSSProperties }) => (
  <div className="cmux-card" style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, padding: "9px 10px", marginBottom: 7, ...style }}>
    {children}
  </div>
)

const CardTitle = ({ children }: { children: React.ReactNode }) => (
  <div className="cmux-card-title" style={{ fontSize: 10, fontWeight: 700, color: C.subtle, textTransform: "uppercase", letterSpacing: 0.6, marginBottom: 7 }}>
    {children}
  </div>
)
...
const Btn = ({ children, onClick, color, disabled, style }: {
  ...
  return (
    <button className="cmux-btn" onClick={onClick} disabled={disabled} style={{
...
const BtnFx = ({ children, onClick, color, disabled, style, toast }: {
  ...
  return (
    <button className="cmux-btn" onClick={handleClick} disabled={disabled || busy} style={{
...
const Select = ({ label, value, onChange, options }: {
  ...
  <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
    <label style={{ fontSize: 10, color: C.muted, textTransform: "uppercase", letterSpacing: 0.5 }}>{label}</label>
    <select className="cmux-field" value={value} onChange={e => onChange(e.target.value)} style={{
      minHeight: 32, padding: "5px 8px", borderRadius: 7, border: `1px solid ${C.border}`,
  ...
  return (
    <div>
      ...
      <Card style={{ textAlign: "center", padding: "12px 10px" }}>
        ...
        <div className="cmux-signal" style={{ fontSize: 32, fontWeight: 900, color: sigColor(sig), letterSpacing: 0.8, lineHeight: 1 }}>
          {sig ?? "—"}
        </div>
        {intel && (
          <>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 8 }}>
        ...
        <select className="cmux-field" value={autoRefreshSec} onChange={e => onSetAutoRefresh(Number(e.target.value))}
    ...
    <div>
      ...
      <Card>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 10, alignItems: "center" }}>
          ...
          {useUsdt ? (
            <input className="cmux-field" type="number" min={1} step={1} value={usdtAmt} onChange={e => setUsdtAmt(Number(e.target.value))}
            ...
            <input className="cmux-field" type="number" min={0.001} step={0.001} value={qty} onChange={e => setQty(Number(e.target.value))}
              style={{ flex: 1, minWidth: 0, minHeight: 34, padding: "5px 8px", borderRadius: 7, border: `1px solid ${C.border}`, background: C.surface, color: C.text, fontSize: 12 }} />
          )}
        </div>
      ...
      <Card>
        ...
        <textarea className="cmux-field" value={command} onChange={e => setCommand(e.target.value)} rows={2}
  ...
  return (
    <div>
      ...
      <Card>
        ...
        <KV k="Extension" v="Binance Cmux + Hermes" />
...
export const CmuxPanel = memo(({
  ...
  if (!open) {
    return (
      <ToastProvider>
      <div className="cmux-collapsed" style={{
  ...
  return (
    <ToastProvider>
    <div className="cmux-panel" style={{
      position: "fixed", top: 0, right: 0, bottom: 0,
      ...
      {/* ── Header (sticky) ── */}
      <div className="cmux-header" style={{
        padding: "8px 10px 0",
        background: `linear-gradient(180deg, ${C.surface} 0%, ${C.bg} 100%)`,
        borderBottom: `1px solid ${C.border}`,
        flexShrink: 0,
      }}>
        <div className="cmux-headline" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 5, gap: 8 }}>
          <div>
            <div style={{ fontSize: 9, color: C.muted, letterSpacing: 0.8, textTransform: "uppercase" }}>Cmux + Hermes</div>
            <div className="cmux-symbol" style={{ fontSize: 19, fontWeight: 900, letterSpacing: 0.2, lineHeight: 1.05 }}>{symbol}</div>
          </div>
        ...
        {/* Tab bar */}
        <div className="cmux-tabs" style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 1 }}>
          {tabs.map(t => (
            <button className="cmux-tab" key={t.id} onClick={() => setTab(t.id)} style={{
      ...
      {/* ── Scrollable content ── */}
      <div className="cmux-content" style={{
      ...
      <style>{`
        ...
        .cmux-panel,
        .cmux-panel * { box-sizing: border-box; }
        .cmux-panel button,
        .cmux-panel input,
        .cmux-panel select,
        .cmux-panel textarea {
