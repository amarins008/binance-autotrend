...
DEFAULT_CONFIG = ROOT / "configs" / "gold.mt5.demo.json"
CMUX_PORT = int(os.getenv("CMUX_PORT", "8130"))
...
LEARNING_TUNABLE_KEYS = (
    "minConfidence",
    "demoAutoMinConfidence",
    "demoQualityMinConfidence",
    "demoQualityMinMomentumPoints",
    "demoBlockRangeEntries",
    "demoLossCooldownAfter",
    "demoLossCooldownSec",
    "demoMaxLossStreakStop",
    "demoReversalCooldownSec",
    "demoStackMinConfidence",
    "demoStackMinMomentumPoints",
    "demoHigherTimeframeConfirm",
    "demoHigherTimeframes",
    "demoHigherTimeframeMinConfirmations",
    "demoHigherTimeframeMinConfidence",
    "maxOpenOrders",
    "maxOpenPositions",
    "maxExposureLots",
    "guardianEarlyExitMinConfidence",
    "guardianEarlyExitMinMomentumPoints",
    "guardianProfitTrailMinConfidence",
    "guardianProfitTrailMinMomentumPoints",
    "hybridMinScore",
    "hybridMinEdge",
...
def _public_ip_status(force: bool = False) -> dict[str, Any]:
    ...
    if not force and cache_fresh:
        return {
            ...
            "publishUrlHint": f"http://{_local_publish_hint()}:8050",
        }
...
def _build_status_payload() -> dict[str, Any]:
    ...
    return {
        ...
        "stale": bool(hermes_down or (isinstance(bot_status, dict) and bot_status.get("stale"))),
    }
...
def dispatch_get(path: str, query: dict[str, list[str]] | None = None) -> tuple[int, dict[str, Any]]:
    ...
    if path == "/bot/precheck-live":
        symbol = str((qs.get("symbol", ["EURUSD"])[0] or "EURUSD")).strip().upper()
...
def dispatch_post(path: str, payload: dict[str, Any] | None = None) -> tuple[int, dict[str, Any]]:
    ...
    if path == "/bot/start":
        ...
        mode = str(data.get("executionMode", "DEMO") or "DEMO").upper()
        precheck_warning = None
        if mode in ("LIVE", "DEMO"):
            sym = str(data.get("symbol", "EURUSD") or "EURUSD").upper()
            if sym in ("AUTO", "SCAN", ""):
                sym = "EURUSD"
