...
HERMES_PORT = int(os.getenv("BACKEND_PORT", "8120"))
HERMES_BASE = f"http://127.0.0.1:{HERMES_PORT}"
HERMES_APP = os.getenv("HERMES_APP", "main_forex:app")
...
def _python_executable() -> str:
    candidates = [
        ROOT / ".venv-mt5" / "Scripts" / "python.exe",
        ROOT / ".venv-mt5" / "bin" / "python",
...
def start(timeout_sec: float = 12.0) -> dict[str, Any]:
    ...
    _ensure_state_dir()
    cmd = [_python_executable(), "-m", "uvicorn", HERMES_APP, "--host", "127.0.0.1", "--port", str(HERMES_PORT)]
...
def status_quick() -> dict[str, Any]:
    ...
    if healthy:
        ...
        health_age = time.time() - float(_HERMES_HEALTH_CACHE.get("ts", 0.0) or 0.0)
...
def autotrade_precheck_live(symbol: str = "EURUSD") -> dict[str, Any]:
    import urllib.error

    query = urllib.parse.urlencode({"symbol": symbol or "EURUSD"})
    last_error = None
