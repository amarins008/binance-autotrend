...
def _maybe_clear_bad_utc_hour_from_config(skip_code: str, skip_msg: str, cfg: dict | None = None) -> dict:
    """Clear a bad UTC hour from config if the skip reason matches."""
    ...
    import re as _re
    parsed_hours = set()
    for match in _re.finditer(r'(?:UTC\s*)?hour\s*(\d{1,2})|bad UTC hour\s*(\d{1,2})', str(skip_msg or ''), _re.IGNORECASE):
        for g in match.groups():
            if g:
                parsed_hours.add(int(g))
    if not parsed_hours:
        return {}
    new_hours = [h for h in bad_hours if int(h) not in parsed_hours]
    if len(new_hours) == len(bad_hours):
        return {}
    cfg['liveBadUtcHours'] = new_hours
    return {'cleared': list(parsed_hours), 'remaining': new_hours}


def _per_symbol_streak_size_mult(symbol: str, cfg: dict | None = None) -> float:
    """Return a size multiplier based on recent per-symbol win/loss streak."""
    if not isinstance(cfg, dict):
        cfg = {}
    ...
    if not sym:
        return 1.0
    try:
        from trading.supervisor_state import AUTO_TRADE
        streak_state = AUTO_TRADE.get('supervisorSizeStreak', {})
        if not isinstance(streak_state, dict):
            streak_state = {}
        sym_data = streak_state.get(sym, {})
        if not isinstance(sym_data, dict):
            return 1.0
        wins = int(sym_data.get('wins', 0) or 0)
        losses = int(sym_data.get('losses', 0) or 0)
        if wins >= 3:
            return min(1.35, 1.0 + (wins - 2) * 0.10)
        if losses >= 2:
            return max(0.35, 1.0 - (losses - 1) * 0.20)
    except Exception:
        pass
    return 1.0
